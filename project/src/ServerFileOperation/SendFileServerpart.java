 ...
 * @author User
 */
public class SendFileServerpart {
    ...
    private File toSend = null;

    public SendFileServerpart(NetworkConnection nc, String clientname, String whichFile) {
        System.out.println("I am in SendFileserverpart constructor");
        try {
            ...
            System.out.println("I am after din,dout in SendFileserverpart");
        } catch (Exception e) {
    ...
    public SendFileServerpart(NetworkConnection nc, String username, String friend_name, String whichFile) {
        System.out.println("I am in SendFileserverpart constructor");
        try {
            din = new DataInputStream(nc.socket.getInputStream());
            dout = new DataOutputStream(nc.socket.getOutputStream());
    ...
    private void file_sending() {
        try {
            if (whichFile.equals("profilepic")) {
                toSend = new File(System.getProperty("user.dir") + "\\facebook\\accounts\\" + clientname + "\\info\\propic\\" + clientname + ".jpg");
            } else if (whichFile.equals("changed_coverphoto")) {
                //if client already chose a cover photo then taht photo will be shown
                try {
                    toSend = new File(System.getProperty("user.dir") + "\\facebook\\accounts\\" + clientname + "\\info\\coverphoto\\" + clientname + ".jpg");
                } catch (Exception ex) {
                    System.out.println("exception in sendfileServerPart :" + ex);
                }
            }else if(whichFile.equals("friend_profile_pic")){
                  toSend = new File(System.getProperty("user.dir") + "\\facebook\\accounts\\" + friend_name + "\\info\\propic\\" + friend_name + ".jpg");
            ...
            try {
                rw = new RandomAccessFile(toSend, "r");
            } catch (Exception ex) {
                System.out.println("Exception in Sendfileserverpart" + ex);
            }
...
//            dout.write(buffer);
            ...
            Logger.getLogger(SendFileServerpart.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
